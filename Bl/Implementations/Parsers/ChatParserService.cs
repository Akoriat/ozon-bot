...
public class ChatParserService : IChatParserService
{
    ...
    public HashSet<string> ExtractNewChats(HashSet<string> chatIds, DateOnly? minDate = null)
    {
        return _gate.RunSync(d =>
        {
            ...
            {
                newlyAddedChats = 0;
                ReadOnlyCollection<IWebElement> allChats = new ReadOnlyCollection<IWebElement>(new List<IWebElement>());
                for (int u = 0; u < 3; u++)
                {
                    try
                    {
                        allChats = container!.FindElements(By.CssSelector("div[class*='index_chat_']"));
                    }
                    catch
                    {
                        Thread.Sleep(500);
                ...
                foreach (var chat in allChats)
                {
                    ...
                    try
                    {
                        rawDate = chat.FindElement(By.CssSelector("span[class*=\"index_chatDate_\"]")).Text.Trim();
                    }
                    catch
                    {
                        continue;
                    ...
                    Thread.Sleep(400);
                    var allMessages = msgContainer!.FindElements(By.CssSelector("div"));
                ...
                ((IJavaScriptExecutor)d).ExecuteScript("arguments[0].scrollTop += 500;", container);
                Thread.Sleep(1500);
            }
    ...
    private static IWebElement? WaitForElement(IWebDriver driver, string cssSelector, int maxAttempts)
    {
    ...
    private static IWebElement? WaitForElementXPath(IWebDriver driver, string selector, int maxAttempts)
    {
        IWebElement? element = null;
        int attempts = 0;
        while (attempts < maxAttempts)
    ...
    private static IWebElement? WaitForElementTagType(IWebElement webElement, string tagName, int maxAttempts)
    {
        IWebElement? element = null;
        int attempts = 0;
        while (attempts < maxAttempts)
    ...
    private static DateOnly ParseDate(string dateText)
    {
        ...
        if (dotCount == 1)
        {
            DateTime dt = DateTime.ParseExact(dateText + "." + DateTime.Today.Year, "dd.MM.yyyy", CultureInfo.InvariantCulture);
            return DateOnly.FromDateTime(dt);
        }
        else if (dotCount == 2)
        {
            DateOnly dt = DateOnly.ParseExact(dateText, "dd.MM.yyyy", CultureInfo.InvariantCulture);
            return dt;
        }
    ...
    public bool SendMessageToClient(string requestId, string message)
    {
        return _gate.RunSync(d =>
        {
            ...
            while (true)
            {
                var allChats = container!.FindElements(By.CssSelector("div[class*=\"index_chat_\"]")).ToList();
                ...
                foreach (var chat in allChats)
                {
                    ...
                    if (chatId == chatModel.ChatId)
                    {
                        ...
                        Thread.Sleep(400);
                        var textArea = WaitForElementTagType(msgContainer!, "textarea", 10);
                        Thread.Sleep(400);
                        textArea!.Click();
    ...
    private static void Retry(Action action, int maxAttempts, int delayMs)
    {
        int attempts = 0;
        while (true)
        {
    ...
    public bool RefreshActiveTopic(string requestId)
    {
        return _gate.RunSync(d =>
        {
            ...
            while (true)
            {
                var allChats = d.FindElements(By.CssSelector("div[class*=\"index_chat_\"]")).ToList();
