...
{
    /// <summary>
    /// Клиент для работы с OpenAI Assistants v2.
    /// Поддерживает работу нескольких процессов с одним thread‑ID без коллизий.
    /// </summary>
    public sealed class ChatGPTClient : IChatGPTClient
    {
        ...
        private static readonly SemaphoreSlim _semaphore = new SemaphoreSlim(initialCount: 2);
            ...
            CancellationToken ct = default)
        {
            ...
            return list?.Data ?? new();             // если null — вернуть пустой список
        }
    ...
    long Created_At);
        ...
        {
            Console.WriteLine($"SendMessageAsync: send message: {userMessage} in gpt");
       ...
       || ex.Message.Contains("temporarily_unavailable");
        ...
        private async Task WaitRunAsync(string threadId, string runId, CancellationToken ct, int pollMs = 1_000, int abortAfterS = 300)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                var resp = await _httpClient.GetAsync(url, ct);
                resp.EnsureSuccessStatusCode();
                var run = JsonConvert.DeserializeObject<Run>(await resp.Content.ReadAsStringAsync(ct));
                if (run == null) throw new("Cannot deserialize Run");
        ...
        private async Task<string> GetAssistantResponseAsync(string threadId, string runId, CancellationToken ct)
        {
            ...
            var assistant = root["data"]?
                .FirstOrDefault(m => m?["role"]?.ToString() == "assistant" &&
                                     m?["run_id"]?.ToString() == runId);

            if (assistant == null)
            {
                throw new("Assistant message not found");
            }
