...
namespace Ozon.Bot.Services
{
    public class TopicHostedService : ResilientBackgroundService
    {
        ...
        private readonly PromtsConfig _promtsConfig;
...
IParsersModeBl parsersModeBl)
        ...
        private async Task HandleReviewAsync(NewDataEntry entry, INewDataRepositoryBl dataRepositoryBl, CancellationToken ct)
        {
            ...
            if (!string.IsNullOrEmpty(reviewEntity.Dialog))
                history = _promtsConfig.ReviewIfHaveDialog
            ...
            {
                history = _promtsConfig.ReviewIfNotHaveDialog
                    .Replace("TOVAR", reviewEntity.Product)
                    .Replace("CLIENTNAME", reviewEntity.Name)
                    .Replace("RATING", reviewEntity.Rating.ToString())
                    .Replace("PHOTO", reviewEntity.Photo != "—" ? "Да" : "Нет")
            ...
            var assType = reviewEntity.Rating > 3 ?
                Bl.Common.Enum.AssistantType.ReviewGoodId :
                Bl.Common.Enum.AssistantType.ReviewBadId;
            ...
            {
                Bl.Common.Enum.AssistantType.ReviewGoodId => $"{green}ОП{reviewEntity.Rating}_{reviewEntity.ReviewDate},{reviewEntity.ReviewTime}",
                Bl.Common.Enum.AssistantType.ReviewBadId => $"{red}ОО{reviewEntity.Rating}_{reviewEntity.ReviewDate},{reviewEntity.ReviewTime}",
            };
        ...
        private async Task HandleQuestionAsync(NewDataEntry entry, INewDataRepositoryBl dataRepositoryBl, CancellationToken ct)
        {
            ...
            if (!string.IsNullOrEmpty(questionEntity.ChatConversation))
            {
                ...
                history = _promtsConfig.QuestionIfHaveDialog
            ...
            {
                history = _promtsConfig.QuestionIfNotHaveDialog
                    .Replace("TOVAR", questionEntity.Product)
                    .Replace("CLIENTNAME", questionEntity.ClientName)
                    .Replace("ARTICLE", questionEntity.Article)
                    .Replace("COMMENT", message);
            ...
            {
                "BR" => Bl.Common.Enum.AssistantType.QuestionsBrId,
                "CH" => Bl.Common.Enum.AssistantType.QuestionsChId,
                "KR" => Bl.Common.Enum.AssistantType.QuestionsKrId,
                "DS" or "DN" => Bl.Common.Enum.AssistantType.QuestionsDsOrDnId,
                _ => Bl.Common.Enum.AssistantType.QuestionsOthersId,
            };
            ...
            string topicName = assType switch
            {
                Bl.Common.Enum.AssistantType.QuestionsBrId => $"{yellow}ВБ_{questionEntity.Date},{questionEntity.Time}",
                Bl.Common.Enum.AssistantType.QuestionsChId => $"{yellow}ВЧ_{questionEntity.Date},{questionEntity.Time}",
                Bl.Common.Enum.AssistantType.QuestionsKrId => $"{yellow}ВК_{questionEntity.Date},{questionEntity.Time}",
                Bl.Common.Enum.AssistantType.QuestionsDsOrDnId => $"{yellow}ВД_{questionEntity.Date},{questionEntity.Time}",
                Bl.Common.Enum.AssistantType.QuestionsOthersId => $"{yellow}ВП_{questionEntity.Date},{questionEntity.Time}",
            };
    ...
    Regex.Replace(src, @"^\s*•?\s*\d{1,2}:\d{2}\s*", "").Trim();
        private async Task HandleChatAsync(NewDataEntry entry, IChatParserBl chatParserBl, INewDataRepositoryBl dataRepositoryBl, CancellationToken ct)
        {
            ...
            var history = _promtsConfig.Chat
            ...
            await _forumTopicService.CreateTopicForRequestAsync(new CreateTopicDto
            {
                ...
                Product = "",
                AssistantType = Bl.Common.Enum.AssistantType.ChatGeneralId,
                TopicName = TopicName,
