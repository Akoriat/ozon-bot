...
{
    public class OzonReviewParserService : IOzonReviewParserService
    {
        private readonly SeleniumGate _gate;
        private readonly WebDriverWait _wait;
        private readonly ILogger<OzonReviewParserService> _logger;
        private readonly string _sellerName;

        //public IWebDriver Driver => _driver;
        public OzonReviewParserService(SeleniumGate gate, ILogger<OzonReviewParserService> logger, IOptions<SellerConfig> sellerOptions)
        {
            _gate = gate;
            _wait = _gate.RunAsync(d => { return Task.FromResult(new WebDriverWait(d, TimeSpan.FromSeconds(10))); }).ConfigureAwait(false).GetAwaiter().GetResult();
            _logger = logger;
        ...
        private ProcessModalFromReviewsParserDto ProcessModal(IWebDriver d, out bool inTopic, out string clientName)
        {
            ...
            //_logger.LogInformation("Поиск модального окна...");
            ...
            //_logger.LogInformation("Модальное окно найдено.");
            ...
            clientName = modal
                ...
                .Text.Trim();
            //_logger.LogInformation("Имя клиента получено: {ClientName}", clientName);
            ...
            try
            {
                ...
                if (reviewNode != null)
                {
                    ...
                    //_logger.LogInformation("Текст отзыва: {ReviewText}", reviewText);
                }
                else
                {
        ...
        public async Task<(List<InTopicModelDto<Review>>, bool)> ParseIteration(DateOnly date, TimeOnly time, CancellationToken ct)
        {
            (List<InTopicModelDto<Review>> result, bool hasMore) = await _gate.RunAsync(async d =>
            {
                return await ParseIterationCore(d, date, time, ct);
            }, ct);

            return (result, hasMore);
        }

        public async Task<(List<InTopicModelDto<Review>>, bool)> ParseIterationCore(IWebDriver d, DateOnly date, TimeOnly time, CancellationToken ct)
        {
            ...
            int timeoutSeconds = 5)
        {
            ...
            while (true)
            {
                ...
                //js.ExecuteScript("arguments[0].scrollTop = arguments[0].scrollHeight;", containerForScroll);
                js.ExecuteScript(@"
    (document.scrollingElement || document.documentElement).scrollTop =
        ...
        public bool ClickButtonCore(IWebDriver d)
        {
            ...
            try
            {
                ...
                var loadMoreButton = _wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(By.CssSelector("button[class*='button-module_button_'][class*='button-module_size-500_'][class*='index_showMoreBtn_']")));
        ...
        {
            hasMore = _gate.RunAsync(async d => { return ClickButtonCore(d); }).GetAwaiter().GetResult();
        }
        ...
        public bool SendMessageToClient(string requestId, string message, string article)
        {
            return _gate.RunAsync(async d =>
            {
                ...
                await ClickCalendarDateCore(d, findDate);
                Thread.Sleep(600);
        ...
        private bool ProcessModalToSendMessage(IWebDriver d, string clientName, string message)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        //_logger.LogWarning(ex, "Ошибка при клике на кнопку отправки сообщения, пробую через JS");
        ...
        public async Task<bool> ClickCalendarDate(DateOnly date)
        {
            return await _gate.RunAsync(async driver =>
            {
                return await ClickCalendarDateCore(driver, date);
            });
        }

        public async Task<bool> ClickCalendarDateCore(IWebDriver driver, DateOnly date)
        {
            try
            {
                ...
                for (var i = 0; i < 24; i++)        // 2 года запаса, чтобы не зациклиться
                {
                    ...
                    // маленькая пауза, чтобы анимация не «сгрызла» клик
                    wait.Until(d => d.FindElements(
                                     By.CssSelector($"button[data-testid='calendar-day'][aria-label='{targetLabel}']"))
                                 .Any() ||            // нужный месяц прогрузился
                                 d.FindElements(By.CssSelector(arrowSelector)).Any()); // стрелка снова кликабельна
                }
